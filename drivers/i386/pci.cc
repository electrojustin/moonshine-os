...
uint32_t compute_config_address(uint8_t bus, uint8_t device, uint8_t func,
                                uint8_t addr) {
  uint32_t config_address = 0;
  config_address = (uint32_t)addr & 0xFC | (uint32_t)func << 8 |
                   (uint32_t)device << 11 | (uint32_t)bus << 16 | 0x80000000;
...
void pci_read_config_header(uint8_t bus, uint8_t device, uint8_t func,
  ...
  for (int i = 0; i < sizeof(struct pci_config_header) / sizeof(uint32_t);
       i++) {
    config_buf[i] =
        pci_read_config_register(bus, device, func, i * sizeof(uint32_t));
  }
}
...
void pci_write_config_header(uint8_t bus, uint8_t device, uint8_t func,
  ...
  for (int i = 0; i < sizeof(struct pci_config_header) / sizeof(uint32_t);
       i++) {
    pci_write_config_register(bus, device, func, i * sizeof(uint32_t),
                              config_buf[i]);
  }
}
...
struct pci_device_list find_pci_devices(void) {
  ...
  ret.config_headers = (struct pci_config_header *)kmalloc(
      alloc_size * sizeof(struct pci_config_header));
      ...
      for (int function = 0; function < 8; function++) {
        if (check_function(bus, device, function)) {
          ...
          if (ret.num_devices > alloc_size) {
            alloc_size *= 2;
            ret.busses =
                (uint8_t *)krealloc(ret.busses, alloc_size * sizeof(uint8_t));
            ret.devices =
                (uint8_t *)krealloc(ret.devices, alloc_size * sizeof(uint8_t));
            ret.functions = (uint8_t *)krealloc(ret.functions,
                                                alloc_size * sizeof(uint8_t));
            ret.config_headers = (struct pci_config_header *)krealloc(
                ret.config_headers,
                alloc_size * sizeof(struct pci_config_header));
          }
          ...
          pci_read_config_header(bus, device, function,
                                 ret.config_headers + (ret.num_devices - 1));
        }
      }
    }
  }
