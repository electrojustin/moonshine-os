...
void enter_lba_address(const struct ide_device& device, uint8_t num_sectors, uint32_t lba) {
...
// These values are the default ports for the IDE controller and will almost never vary.
// If you suspect otherwise, you can consult the PCI config of the controller.
struct ide_device devices[4] = { {.io_base_port=0x01F0, .control_base_port=0x03F6, .device_select=0},
