...
namespace {
	...
	__attribute__((interrupt)) void unregistered_interrupt(struct interrupt_frame* frame) {
		lib::std::printk((char*)"Unregistered interrupt\n");

		// We don't actually know the interrupt number, but this is just to figure out which
		// PIC we should send the end of interrupt command to. We'll just assume we need to
		// send it to both of them.
		arch::interrupts::end_interrupt(0xFF);
	}

	void register_interrupt_handler_internal(uint8_t interrupt_number, uint8_t type, uint8_t privilege, void* interrupt_handler) {
		...
		idt[interrupt_number].flags = 0b10000000 | type | (privilege << 5); // The highest bit means "enabled" so it should always be set.
	...
	void register_error_interrupts(void) {
		...
		register_interrupt_handler_internal(7, TRAP_GATE, 0, (void*)device_not_available);
		...
		register_interrupt_handler_internal(13, TRAP_GATE, 0, (void*)general_protection_fault);
		register_interrupt_handler_internal(14, TRAP_GATE, 0, (void*)page_fault);
		register_interrupt_handler_internal(16, TRAP_GATE, 0, (void*)floating_point_exception);
...
void register_interrupt_handler(uint8_t interrupt_number, uint8_t type, uint8_t privilege, void* interrupt_handler) {
	register_interrupt_handler_internal(interrupt_number, type, privilege, interrupt_handler);
	flush_idt();
}
...
void initialize_interrupts(void) {
	...
	for (int i = 0; i < NUM_INTERRUPTS; i++) {
		register_interrupt_handler_internal(i, INTERRUPT_GATE, 0, (void*)unregistered_interrupt);
	}
