...
struct memory_table detect_memory(void) {
	struct memory_info_entry* initial_memory_map = (struct memory_info_entry*)memory_map_initial_address;
	struct memory_table ret;
	ret.num_memory_entries = call_bios_memory((uint32_t)initial_memory_map & 0xFFFF);
	ret.entries = initial_memory_map;
	lib::std::printk("Detection: %x   %x\n", initial_memory_map[0].addr, initial_memory_map[0].length);
	return ret;
}
