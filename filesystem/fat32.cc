...
uint32_t read_clusters(uint32_t cluster, uint8_t* buf, size_t len) {
	...
	while(bytes_read < len) {
		read_sectors(*device, tmp_buf, cluster_sectors, cluster_start + (cluster-2)*cluster_sectors);
		...
		if ((file_allocation_table[cluster] & 0x0FFFFFFF) >= (uint32_t)END_OF_FILE_CLUSTER) {
			break;
		} else {
...
uint32_t parse_long_filename(char* buf, char** filename) {
	...
	size_t parts_size = 1;
	size_t parts_len = 0;
	struct lfn_part* parts = (struct lfn_part*)kmalloc(parts_size*sizeof(struct lfn_part));
	int i = 0;
	while (lfn_entries[i].attribute == LONG_FILE_NAME) {
		...
		if (parts_len > parts_size) {
			parts_size *= 2;
			parts = (struct lfn_part*)krealloc(parts, parts_size*sizeof(struct lfn_part));
		}
...
uint32_t find_cluster(char* path, uint32_t current_cluster=root_dir_cluster) {
	...
	struct directory_table_entry entry = find_directory_entry(path, current_cluster);
	if (!entry.filename[0]) {
...
void flush_fat(void) {
	...
	for (int i = 0; i < num_fats; i++) {
		write_sectors(*device, (uint8_t*)file_allocation_table, fat_sectors, fat_lba);
		fat_lba += fat_sectors;
	}
}
	...
	for (int i = search_start; i < fat_size / sizeof(uint32_t); i++) {
		if(!file_allocation_table[i]) {
			if (len <= cluster_size) {
				...
				uint32_t next_cluster = alloc_cluster(len - cluster_size, search_start+1);
...
int create_long_filename_entries(char* filename, uint8_t checksum, struct long_filename_entry** output_entries) {
	int len = strlen(filename);
	int num_entries = (len+1) / 13;
	if (13*(len+1) != num_entries) { // We wanna round up here
		num_entries++;
	}
	struct long_filename_entry* entries = (struct long_filename_entry*)kmalloc(num_entries * sizeof(struct long_filename_entry));
...
char add_directory_entry(char* dir_path, char* name, uint32_t attributes, size_t len, uint32_t cluster) {
	...
	int num_long_filename_entries = create_long_filename_entries(name, checksum, &long_filename_entries);
	...
	memcpy((char*)long_filename_entries, (char*)(dir_table+i-num_long_filename_entries), num_long_filename_entries*sizeof(struct long_filename_entry));
	memcpy(eight_three_filename, dir_table[i].filename, sizeof(dir_table[i].filename));
...
void init_fat32(struct ide_device& _device, struct partition& partition) {
	...
	read_sectors(*device, (uint8_t*)file_allocation_table, fat_sectors, fat_start);
...
struct directory ls_fat32(char* path) {
	...
	ret.children = (struct directory_entry*)kmalloc(num_children*sizeof(struct directory_entry));
	int child_index = 0;
	for (int i = 0; i < cluster_size/sizeof(struct directory_table_entry); i++) {
		if (!dir_table[i].filename[0]) {
			break;
		...
		if (dir_table[i].filename[0] == DELETED_DIR_ENTRY) {
			...
			i += parse_long_filename((char*)&dir_table[i], &ret.children[child_index].name);
			child_index++;
		} else{
			ret.children[child_index].name = unpack_eight_three_filename(dir_table[i].filename);
			ret.children[child_index].read_only = !!(dir_table[i].attributes & READ_ONLY_FLAG);
...
char mkdir_fat32(char* path) {
	...
	if (!add_directory_entry(dir_path, name, DIRECTORY_FLAG, cluster_size, new_cluster)) {
	...
	if (!add_directory_entry(path, ".", DIRECTORY_FLAG, cluster_size, new_cluster)) {
		// This should never happen and will result in filesystem corruption.
		panic("Failed to allocate new directory!");
	}

	if (!add_directory_entry(path, "..", DIRECTORY_FLAG, cluster_size, parent_cluster)) {
