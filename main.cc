...
void kernel_main(multiboot_info_t* multiboot_info, unsigned int magic) {
	...
	io::vga_initialize();
 	lib::std::setcolor(io::vga_entry_color(io::VGA_COLOR_LIGHT_GREY, io::VGA_COLOR_BLACK));

	// Setup identity paging and a flat GDT.
	arch::memory::map_memory_range(base_page_directory, base_page_table, 0, 0, (uint32_t)1024*1023*4096, arch::memory::kernel_read_write);
	...
	// Use the memory map provided to us by GRUB to initialize our dynamic memory management.
	lib::std::printk("Mapping memory...\n");
	struct memory_info_entry* memory_table = (struct memory_info_entry*)(multiboot_info->mmap_addr+4);
	uint16_t num_mmap_entries = multiboot_info->mmap_length / sizeof(memory_info_entry);
	for (int i = 0; i < num_mmap_entries; i++) {
		lib::std::printk("Segment start: %x  Segment length: %x  Segment type: %x\n", memory_table[i].addr_low, memory_table[i].length_low, memory_table[i].type);
		if (memory_table[i].type == 1) {
			lib::std::initialize_allocator((void*)memory_table[i].addr_low, (void*)((char*)memory_table[i].addr_low + memory_table[i].length_low));
		}
	}
	...
	// Initialize the programmable interrupt controller (PIC) to use interrupts 0x20-0x2F.
