...
// The output_buffer is a ring buffer of rows. Each entry is 2 bytes: 1 for
// color, 1 for character. top_index points to the top of the page, bottom_index
// points to the bottom. All arithmetic done on them is modulo the height of the
// page. terminal_column is simply the column.
...
void print_stack_trace(void *current_ebp) {
  ...
  // Save ebp
  asm volatile("mov %%ebp, %0" : "=r"(original_ebp));

  // x86 ABI says the last ebp is stored at the memory address the current one
  // points to. The stored eip is stored directly above ebp. This prints the eip
  // and uses ebp to unroll the stack.
  ...
  asm volatile("mov %0, %%ebp" : : "r"(original_ebp));
}
