...
void execute_new_process(void) {
	...
	asm volatile(
		...
		"iret"
		:
		: "m" (esp), "m" (code_virtual_start), "i" (USER_CODE_SELECTOR), "i" (USER_DATA_SELECTOR));
}
...
char spawn_new_process(char* path, struct process_memory_segment* segments, uint32_t num_segments, void (*entry_address)(void), char* working_dir) {
	...
	for (int i = 0; i < new_proc->num_segments; i++) {
		...
		alloc_size = (alloc_size & (PAGE_SIZE-1)) ? ((alloc_size/PAGE_SIZE)+1)*PAGE_SIZE : alloc_size;
	...
	// So we just add a temporary TLS with the right syscall info :P
	// TODO: maybe file a bug report?
	struct tls_segment* temp_tls = (struct tls_segment*)kmalloc(sizeof(struct tls_segment));
	...
	*(uint32_t*)((uint32_t)stack_segment->actual_address + 0x10) = (uint32_t)(stack_segment->virtual_address + raw_syscall_copy_offset); // Got this magic number from a disassembly dump of libc
	...
	new_proc->page_dir = (uint32_t*)kmalloc_aligned(1024*sizeof(uint32_t), PAGE_SIZE);
	memcpy((char*)base_page_directory, (char*)new_proc->page_dir, 2*sizeof(uint32_t));
...
void execute_processes(void) {
	while (process_list) {
		if (process_list->process_state == STOPPED) {
			...
			process_list = process_list->next;
			while (process_list != current_proc && process_list->process_state == WAITING) {
				process_list = process_list->next;
			}
