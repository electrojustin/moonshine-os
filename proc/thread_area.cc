...
uint32_t set_thread_area(uint32_t tls_segment_addr, uint32_t reserved2,
  ...
  if (tls_copy.gdt_index - TLS_ENTRY_OFFSET >= 0) {
    ...
    uint32_t new_index =
        current_process->num_tls_segments - 1 + TLS_ENTRY_OFFSET;
    current_process->tls_segment_index = new_index - TLS_ENTRY_OFFSET;

    tls_copy.gdt_index = new_index;

    current_process->tls_segments = (struct tls_segment *)krealloc(
        current_process->tls_segments,
        current_process->num_tls_segments * sizeof(struct tls_segment));
    current_process->tls_segments[current_process->num_tls_segments - 1] =
        tls_copy;
