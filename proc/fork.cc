...
uint32_t fork(uint32_t reserved1, uint32_t reserved2, uint32_t reserved3, uint32_t reserved4, uint32_t reserved5) {
	...
	memcpy((char*)parent_proc->tls_segments, (char*)new_proc->tls_segments, sizeof(struct tls_segment) * new_proc->num_tls_segments);
...
uint32_t clone(uint32_t flags, uint32_t stack_addr, uint32_t parent_tid_addr, uint32_t tls, uint32_t child_tid_addr) {
