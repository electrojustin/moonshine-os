...
char load_elf(char *path, int argc, char **argv, char *working_dir) {
  ...
  if (header->ident[0] != 0x7F || header->ident[1] != 'E' ||
      ...
      header->type != EXECUTABLE_TYPE || header->machine != MACHINE_TYPE_X86) {
    kfree(file_buf);
    return 0;
  }
  ...
  struct process_memory_segment *process_segments =
      (struct process_memory_segment *)kmalloc(
          num_loadable_segments * sizeof(struct process_memory_segment));
  int process_segment_index = 0;
  for (int i = 0; i < num_segments; i++) {
    if (segment_table[i].type == LOADABLE_SEGMENT) {
      process_segments[process_segment_index].virtual_address =
          (void *)(segment_table[i].virtual_address);
      process_segments[process_segment_index].segment_size =
      ...
      if (segment_table[i].disk_size) {
        process_segments[process_segment_index].source =
            file_buf + segment_table[i].offset;
        process_segments[process_segment_index].disk_size =
  ...
  // Spawn the process
  char ret = spawn_new_process(path, argc, argv, process_segments,
                               num_loadable_segments, (void (*)())header->entry,
                               working_dir);
