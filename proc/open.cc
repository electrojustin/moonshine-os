...
constexpr uint32_t AT_FDCWD = (~100)+1; // This is supposed to be -100 signed but we assume unsigned ints :\

uint32_t open_internal(struct process* current_process, char* path, uint32_t flags, uint32_t mode) {
...
uint32_t open(uint32_t path_addr, uint32_t flags, uint32_t mode, uint32_t reserved1, uint32_t reserved2) {
...
uint32_t openat(uint32_t directory_fd, uint32_t path_addr, uint32_t flags, uint32_t mode, uint32_t reserved) {
	struct process* current_process = get_currently_executing_process();
	uint32_t* page_dir = current_process->page_dir;
	char* relative_path = make_virtual_string_copy(page_dir, (char*)path_addr);
...
uint32_t access(uint32_t path_addr, uint32_t mode, uint32_t reserved1, uint32_t reserved2, uint32_t reserved3) {
	return 0;
}
