...
uint32_t llseek(uint32_t file_descriptor, uint32_t offset_high, uint32_t offset_low, uint32_t result_addr, uint32_t whence) {
	// 64 bit offsets are unsupported
	return lseek(file_descriptor, offset_low, whence, 0, 0);
}
